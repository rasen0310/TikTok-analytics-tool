-- 修正済み完全なTikTok分析ツールのデータベーススキーマ
-- 実行順序: 1. このファイルを最初に実行してください
...
-- tiktok_accounts RLSポリシー（既存があれば削除してから作成）
...
BEGIN
  ...
  AND table_name IN ('tiktok_accounts', 'oauth_states', 'tiktok_videos');
...
BEGIN
  ...
  AND tablename IN ('tiktok_accounts', 'oauth_states', 'tiktok_videos');
